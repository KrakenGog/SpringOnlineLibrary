...
import com.onlib.core.dto.BookInfoDto;
...
public class BookController {
    ...
    public ResponseEntity<BookInfoDto> getBook(
            @RequestParam long id) {
        try {
            return new ResponseEntity<>(
                    new BookInfoDto(bookService.getBookById(id)),
                    HttpStatus.OK);
    ...
    public ResponseEntity<byte[]> getBookEpubFile(
        ...
        try {
            return new ResponseEntity<>(
                    bookFileProvider.getEpubFile(id),
                    headers,
                    HttpStatus.OK
            );
    ...
    public ResponseEntity<String> addBook(
        ...
        try {
            bookService.AddBook(
                    name,
                    description,
                    authorService.getAuthorsWithIds(authorsId),
                    file.getBytes()
            );
        } catch (IOException ex) {
